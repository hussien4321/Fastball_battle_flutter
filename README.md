...
**Key libraries** · Flutter `CustomPainter` / `dart:ui` · `audioplayers` · `flutter_local_notifications` · `firebase_admob` · `flutter_inapp_purchase` · `path_provider` · `page_view_indicator`

No game engine, no sprite library — just Canvas
...
| **One collision, two reactions** | The batter and pitcher react to the same event at different points on one shared timeline, so a hit reads correctly from both sides of the screen. | [↓](#2-one-timeline-two-offsets) |
| **Characters without code** | A character is a JSON description of image sequences. Adding one is an asset drop and a data entry — no new painter, no new branch. | [↓](#3-characters-are-data) |
...
- **`GamePainter`'s constructor takes 20 positional parameters.** It works and it's fast, but the call site is unreadable and trivially easy to mis-order. A single immutable frame-state object is the obvious fix.
- **No tests.** The frame-index maths — window remapping, the `0.99` clamp, the hit windows — is pure arithmetic with clear boundaries and would be cheap to cover.
- **Pre-null-safety.** Nullable images are guarded by convention rather than the type system.
- **All frames decode at startup.** Fine for this roster size; it wouldn't scale to a large one without streaming or atlasing.
