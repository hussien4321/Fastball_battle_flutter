...
class GamePage extends StatefulWidget {
  ...
  GamePage(this.parentContext, this.bgmController, this.isMusicOn,
      this.tonesPlayer, this.isTonesOn, this.allCharsUnlocked);
...
class _GamePageState extends State<GamePage>
  ...
  static final int MACHINE_FIXED_DELAY =
      1500; //the minimum ms time that the machine takes to send a new obstacle
  static final int MACHINE_OFFSET_DELAY =
      1500; //the additional ms time added to the machine fixed delay to prevent predictability

  Animation<int> obstacleAnimation;
  AnimationController obstacleAnimationController;

  static final int OBSTACLE_DURATION =
      300; //ms time for obstacle to be first to passing by user
  static final int CAN_HIT_OBSTACLE = ((OBSTACLE_DURATION * 2) / 3)
      .floor(); //ms time for obstacle to be first to passing by user

  Animation<int> obstacleDeathAnimation;
  AnimationController obstacleDeathAnimationController;

  static final int OBSTACLE_DEATH_DURATION =
      1000; //ms time for obstacle to be first to passing by user
  ...
  static final int TIME_INVALID_AFTER_HIT =
      500; //ms time disabled after clicking hit

  AnimationController flashingTextAnimationController;

  static final int FLASHING_TEXT_DURATION =
      2000; //the time for one appear and disappear of text

  AnimationController gameStartAnimationController;

  static final int GAME_START_TEXT_DURATION =
      3000; //the minimum time the user should be shown the message on the load screen before game starts

  AnimationController waitForAdsController;

  static final int WAIT_FOR_ADS_DURATION =
      1500; //the minimum time the user should be prevented from starting a new game, so the ad can load appropriately
  ...
  void initControllers() {
    machineAnimationController =
        new AnimationController(duration: new Duration(), vsync: this);
    obstacleAnimationController = new AnimationController(
        duration: new Duration(milliseconds: OBSTACLE_DURATION), vsync: this);
    obstacleDeathAnimationController = new AnimationController(
        duration: new Duration(milliseconds: OBSTACLE_DEATH_DURATION),
        vsync: this);
    inputAnimationController = new AnimationController(
        duration: new Duration(milliseconds: TIME_INVALID_AFTER_HIT),
        vsync: this);
    flashingTextAnimationController = new AnimationController(
        duration: new Duration(milliseconds: FLASHING_TEXT_DURATION),
        vsync: this);
    gameStartAnimationController = new AnimationController(
        duration: new Duration(milliseconds: GAME_START_TEXT_DURATION),
        vsync: this);
    waitForAdsController = new AnimationController(
        duration: new Duration(milliseconds: WAIT_FOR_ADS_DURATION),
        vsync: this);

    machineAnimation =
        IntTween(begin: 0, end: 0).animate(machineAnimationController);
    obstacleAnimation = IntTween(begin: OBSTACLE_DURATION, end: 0)
        .animate(obstacleAnimationController);
    obstacleDeathAnimation = IntTween(begin: OBSTACLE_DEATH_DURATION, end: 0)
        .animate(obstacleDeathAnimationController);
    inputAnimation = IntTween(begin: TIME_INVALID_AFTER_HIT, end: 0)
    ...
    machineAnimationController.addStatusListener((status) {
      if (status == AnimationStatus.completed) {
        ...
        if (widget.isTonesOn) {
          widget.tonesPlayer.play(
              objectsLoader.getThrowSound(shootBullets: enemy.shootsBullets));
        }
      ...
      if (status == AnimationStatus.completed) {
        if (obstacleIsHit) {
          ...
          if (newScore > highScore) {
            ...
            nextUnlockable = objectsLoader.calculateNextUnlockable(
                highScore, widget.allCharsUnlocked);
            newHighScore = true;
          }
          setState(() {
    ...
    obstacleDeathAnimationController.addListener(() {
      if (obstacleDeathAnimation.value / OBSTACLE_DEATH_DURATION < 0.7 &&
          !playedEnemyHurtSound &&
          obstacleIsHit) {
        if (widget.isTonesOn) {
  ...
  void loadImages() async {
    ...
    if (enemy.shootsBullets) {
      String revSrc =
          enemy.weaponSrc.substring(0, enemy.weaponSrc.length - 4) + '_rev.png';
      ballRevImage = await objectsLoader.loadImage(revSrc, context);
    }

    collisionImages = await objectsLoader.loadCollisionImages(context);

    enemyIdleImages = await objectsLoader.loadEnemyImages(
        enemy.id, ENEMY_ACTION_TYPE.IDLE, context);
    enemyInputImages = await objectsLoader.loadEnemyImages(
        enemy.id, ENEMY_ACTION_TYPE.THROW, context);
    enemyHurtImages = await objectsLoader.loadEnemyImages(
        enemy.id, ENEMY_ACTION_TYPE.HURT, context);

    charIdleImages = await objectsLoader.loadCharImages(
        char.id, CHAR_ACTION_TYPE.IDLE, context);
    charInputImages = await objectsLoader.loadCharImages(
        char.id, CHAR_ACTION_TYPE.SWING, context);
    charHurtImages = await objectsLoader.loadCharImages(
        char.id, CHAR_ACTION_TYPE.HURT, context);
    charDeathImages = await objectsLoader.loadCharImages(
        char.id, CHAR_ACTION_TYPE.DEATH, context);
  ...
  void startMachine() {
    ...
    machineAnimation =
        IntTween(begin: rand, end: 0).animate(machineAnimationController);
  ...
  void triggerInput() {
    if (canInput && gameInProgress) {
      if (widget.isTonesOn && obstacle_status != OBSTACLE_STATUS.DEATH) {
        widget.tonesPlayer
            .play(objectsLoader.getSwingSound(), volume: 0.5)
            .then((controller) {
          swingController = controller;
        });
      }
      if (obstacleAnimation.value != 0 &&
          obstacleAnimation.value <= CAN_HIT_OBSTACLE) {
        obstacleIsHit = true;
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Container(
        decoration: stage == null
            ? null
            : BoxDecoration(
                image: new DecorationImage(
                  image: new AssetImage(stage.src,
                      bundle: DefaultAssetBundle.of(context)),
                  fit: BoxFit.fill,
                ),
              ),
        child: (loading || forcedLoading)
            ? Center(child: Text('Tap screen to swing!'))
            : Stack(
                children: <Widget>[
                  SizedBox.expand(
                    child: Container(
                    ...
                    children: <Widget>[
                      ...
                      Text(
                        newHighScore
                            ...
                            : gameInProgress
                                ? ''
                                : (nextUnlockable == 10000000 ||
                                        widget.allCharsUnlocked
                                    ? 'Good game!'
                                    : 'Next unlockable at $nextUnlockable points'),
  ...
  double machineTimeAsPercentage() {
    return machineAnimation.value == 0
        ? 0.0
        : machineAnimation.value /
            machineAnimationController.duration.inMilliseconds;
  }

  double obstacleTimeAsPercentage() {
    return obstacleAnimation.value == 0
        ? 0.0
        : obstacleAnimation.value / OBSTACLE_DURATION;
  }

  double obstacleDeathTimeAsPercentage() {
    return obstacleDeathAnimation.value == 0
        ? 0.0
        : (obstacleDeathAnimation.value / OBSTACLE_DEATH_DURATION);
  }

  double inputTimeAsPercentage() {
    return inputAnimation.value == 0
        ? 0.0
        : (inputAnimation.value == TIME_INVALID_AFTER_HIT
            ? 0.99
            : (inputAnimation.value / TIME_INVALID_AFTER_HIT));
  }
}
