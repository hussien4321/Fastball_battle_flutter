...
import '../models/action.dart' as Action;
...
enum AVATAR_TYPE { Character, Enemy }

enum CHAR_ACTION_TYPE { IDLE, SWING, HURT, DEATH }
enum ENEMY_ACTION_TYPE { IDLE, THROW, HURT }
...
class ObjectsLoader {
  ...
  static final String NEW_HIGH_SCORE_JINGLE =
      "music/new_high_score_post_game_jingle.wav";
  ...
  Action.Action _loadCharAction(Character char, CHAR_ACTION_TYPE actionType) {
    switch (actionType) {
  ...
  bool checkNewChars(int newHighScore, int lastScore) {
    return _characters.any((char) {
      return newHighScore >= char.unlockThreshold &&
          lastScore < char.unlockThreshold;
    });
  }

  bool checkNewEnemies(int newHighScore, int lastScore) {
    return _enemies.any((enemy) {
      return newHighScore >= enemy.unlockThreshold &&
          lastScore < enemy.unlockThreshold;
    });
  }

  bool checkNewStages(int newHighScore, int lastScore) {
    return _stages.any((stage) {
      return newHighScore >= stage.unlockThreshold &&
          lastScore < stage.unlockThreshold;
    });
  }
  ...
  int calculateNextUnlockable(int newHighScore, bool allCharsUnlocked) {
    ...
    _characters.forEach((char) {
      if (char.unlockThreshold > newHighScore &&
          char.unlockThreshold < nextUnlockable) {
        nextUnlockable = char.unlockThreshold;
      }
    });
    _enemies.forEach((enemy) {
      if (enemy.unlockThreshold > newHighScore &&
          enemy.unlockThreshold < nextUnlockable) {
        nextUnlockable = enemy.unlockThreshold;
      }
    });
    _stages.forEach((stage) {
      if (stage.unlockThreshold > newHighScore &&
          stage.unlockThreshold < nextUnlockable) {
  ...
  Future<List<UI.Image>> loadCharImages(
      int id, CHAR_ACTION_TYPE actionType, BuildContext context) async {
    List<UI.Image> imagesList = [];

    Character currChar = _characters.where((char) => char.id == id).toList()[0];

    Action.Action currAction = _loadCharAction(currChar, actionType);
  ...
  Action.Action _loadEnemyAction(Enemy enemy, ENEMY_ACTION_TYPE actionType) {
    switch (actionType) {
  ...
  Future<List<UI.Image>> loadEnemyImages(
      int id, ENEMY_ACTION_TYPE actionType, BuildContext context) async {
    List<UI.Image> imagesList = [];

    Enemy currEnemy = _enemies.where((enemy) => enemy.id == id).toList()[0];

    Action.Action currAction = _loadEnemyAction(currEnemy, actionType);
  ...
  Future<List<UI.Image>> loadCollisionImages(BuildContext context) async {
    ...
    for (int i = 11; i >= 0; i--) {
      String num = i < 10 ? '0$i' : '$i';
      UI.Image temp =
          await loadImage("assets/other/collision/Smoke_0$num.png", context);
      imagesList.add(temp);
    }
    return imagesList;
  }
  ...
  Future<UI.Image> loadImage(String link, BuildContext context) async {
    ByteData bd =
        await DefaultAssetBundle.of(context).load(link); //.then( (bd) {
